...
def generate_pdf(df):
    ...
    for proj in df['პროექტის დასახელება'].unique():
        ...
        if not debtors.empty:
            ...
            pdf.cell(0, 8, f"პროექტის ჯამური ვალი: {debtors['ვალები'].sum():,.2f} ₾", ln=True)
        ...
        if not advances.empty:
            ...
            pdf.set_font('DejaVu', size=9)
            pdf.cell(0, 8, f"პროექტის ჯამური ავანსი: ({advances['ავანსები'].sum():,.2f}) ₾", ln=True)
...
if file1 and file2:
    ...
    # მონაცემების გასუფთავება (უზრუნველყოფს სწორ ჯამებს)
    ...
    # --- პრევიუ ეკრანზე ---
    st.markdown("---")
    st.subheader("📋 მონაცემების პრევიუ პროექტების მიხედვით")
    
    for p in df1['პროექტის დასახელება'].unique():
        with st.expander(f"📁 პროექტი: {p}", expanded=False):
            p_df = df1[df1['პროექტის დასახელება'] == p]
            st.write("**მევალეები:**")
            st.dataframe(p_df[p_df['ვალები'] > 0][['სახელი გვარი', 'პირადი ნომერი', 'ვალები', 'ტელეფონი', 'შენიშვნა']])
            st.write("**ავანსები:**")
            st.dataframe(p_df[p_df['ავანსები'] > 0][['სახელი გვარი', 'პირადი ნომერი', 'ავანსები']])
    ...
    if st.button("🚀 PDF დოკუმენტის გენერირება"):
        try:
            ...
            st.download_button("📥 გადმოწერეთ დასრულებული PDF", data=bytes(pdf_bytes), file_name="Accounting_Report_AC.pdf", mime="application/pdf")
